...
def training_process(rank, world_size, device):
    ...
    for epoch in range(epochs):
        for batch, (image, text) in enumerate(dataset):
            # Calculate features
            image_list = torch.chunk(image, batch_split)
            text_list = torch.chunk(text, batch_split)
            image_features_list = []
            text_features_list = []
            with torch.no_grad():
            ...
            optimizer.zero_grad()
            total_loss = 0
            for index, (image, text) in enumerate(zip(image_list, text_list)):
                image_feature, text_feature = model_ddp(image, text)
                image_feature_list = image_features_list.copy()
                text_feature_list = text_features_list.copy()
                image_feature_list[index] = image_feature
                text_feature_list[index] = text_feature
